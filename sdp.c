...
struct order{
    ...
    int number_of_food;
    int cost;
}p[10000]; // Declare an array of orders
int order_count = 0, served_count = 0, waiting_now = 0; // Initialize order count variables
// Function for taking order
int order ()
{
    int n, in, qu;

    // Ask for the customer's name and assign an order number
    printf ("please enter your name: ");
    gets (p[order_count].name);
    p[order_count].order_number = 10000+order_count;
    // Call the items function to display menu items
    items();
    // Initialize the number of food items and cost for the order
    ...
    scanf ("%d", &n);
    getchar ();
    // Loop through each item and get its details
    while (n--){
        printf ("\nEnter chosen item code\n");
        scanf ("%d", &in);
        getchar();
        switch(in){
            // Get the details for a burger order
            case 11 : {
...
int main ()
{
    printf ("\t\t     Welcome To BUBT Resturant\n");
    while (1){
        printf ("1. Food Menu \n2. Serve\n3. Display \n4. Exit the program\n");
